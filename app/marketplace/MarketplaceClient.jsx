...
function OpportunityCard({ item }) {
  ...
  return (
    <article className="card opp-card marketplace-card-v41 marketplace-card-v116">
      ...
      <div className="marketplace-metrics-v41 marketplace-metrics-v116">
        <div><span>القيمة</span><b>{item.value}</b></div>
        <div><span>الإيرادات</span><b>{item.revenue}</b></div>
        <div><span>الربح</span><b>{item.profit}</b></div>
      </div>

      <div className="marketplace-card-actions marketplace-card-actions-v51 marketplace-card-actions-v116">
        <Link className="btn btn-primary" href={`/opportunity/${slug}`}>عرض التفاصيل</Link>
