...
// روابط قسم "روابط المنصة" في القائمة الجانبية — بدون "أضف مشروعك" و"أرسل اهتمامك"
const drawerLinks = [
  ['/marketplace', 'الفرص'],
  ['/how-it-works', 'كيف تعمل المنصة؟'],
  ['/blog', 'المدونة'],
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    document.body.classList.toggle('drawer-open-v47', open);
    return () => { document.body.style.overflow = ''; document.body.classList.remove('drawer-open-v47'); };
  }, [open]);
        ...
        <div className="nav-actions nav-actions-v47">
          {session ? (
            <>
              ...
              <Link className="btn btn-secondary" href="/messages">محادثاتي</Link>
      ...
      <div className={`nav-drawer nav-drawer-v47 ${open ? 'is-open' : ''}`} aria-hidden={!open}>
        ...
        <div className="nav-drawer-panel nav-drawer-panel-v47" role="dialog" aria-label="القائمة">
          ...
          <div className="nav-drawer-actions nav-drawer-actions-v47">
            {session ? (
              <>
                <Link className="btn btn-secondary" href="/messages" onClick={close}>محادثاتي</Link>
